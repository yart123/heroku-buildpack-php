	...
	context "with an index.php that allows for different execution times" do
		['apache2', 'nginx'].each do |server|
			context "running the #{server} web server" do
				...
				it "gracefully shuts down when the leader process receives a SIGQUIT" do
					app.deploy do |app|
						...
						cmd = "heroku-php-#{server} & pid=$! ; sleep 5; curl \"localhost:$PORT/index.php?wait=5\" & sleep 2; kill -QUIT $pid; wait $pid"
						output = app.run(cmd)
